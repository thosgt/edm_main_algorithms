...
class MultiHeadedAttention(nn.Module):
    def __init__(self, input_size, output_size, num_heads, drop_prob):
        ...
        self.input_linears = clone(nn.Linear(input_size, output_size), 3) # don't you want to remove the bias here
...
class SAKT(nn.Module):
    ...
    def __init__(self, num_items, embed_inputs, embed_size, hid_size, num_heads, encode_pos, drop_prob):
        ...
        self.exercise_embedding = nn.Embedding(num_items + 1, embed_size, padding_idx=0)
        self.out = nn.Linear(hid_size, num_items)
        
    def forward(self, inputs):
        exercise_ids = torch.clamp(inputs - self.num_items, min=0)
        ...
        out = self.attn(embeds, embeds, exercise_embeds, mask)
        return self.out(out)